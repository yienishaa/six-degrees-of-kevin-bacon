					...
					if (!result.hasNext()) {
						...
						StatementResult pathResult = tx.run(
								"MATCH (a:Actor {actorId:$actorId}), (kb:Actor {actorId:$kevinBaconId}), "
										+ "p=shortestPath((a)-[:ACTED_IN*]-(kb)) " + "RETURN nodes(p) AS path",
								parameters("actorId", actorId, "kevinBaconId", kevinBaconId));
						if (pathResult.hasNext()) {
							Record record = pathResult.next();
							List<Object> pathNodes = record.get("path").asList();
							List<String> baconPath = new ArrayList<>();
							for (Object node : pathNodes) {
								JSONObject nodeJson = new JSONObject((String) node);
								if (nodeJson.has("actorId")) {
									baconPath.add(nodeJson.getString("actorId"));
								} else if (nodeJson.has("movieId")) {
									baconPath.add(nodeJson.getString("movieId"));
								}
							}
